...
public class BSocketAppender extends AppenderSkeleton {
	 ...
	 */
	public BSocketAppender(InetAddress address, int port) {
		errorHandler = new MyError();
	 ...
	 */
	public BSocketAppender(String host, int port) {
		errorHandler = new MyError();
		this.port = port;
	...
	void connect(InetAddress address, int port) {
		...
		try {
			...
			if (!firstConnect && reconnectionDelay > 0) {
	...
	/**
	 * The <b>ReconnectionDelay</b> option takes a positive integer representing the
	...
	public class MyError extends OnlyOnceErrorHandler {
		...
		public void onFailed(String message) {
			if (message != null) {
				System.out.println("BSocketAppender failed:message=" + message);
			}
			Glog.setFailed();
		}
	}
