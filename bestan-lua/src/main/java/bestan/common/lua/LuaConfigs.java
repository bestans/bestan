...
//	}
	...
	public static boolean loadConfig(String rootPath, String[] packageNames) {
		try {
			...
			throw new LuaException("LuaConfigs init failed.error=" + e.getMessage());
			//Glog.trace("LuaConfigs init failed.error={}", e.getMessage());
			//return false;
		}
		...
		try {
			...
			try
			{
				...
				if (configInstance.get(null) != null) {
					throw new RuntimeException("instance has value, maybe have more than one config:" + cls.getSimpleName());
				}
			...
			throw new LuaException("LuaConfigs failed.cls=" + e.getClass().getSimpleName() + ",message=" + e.getMessage());
